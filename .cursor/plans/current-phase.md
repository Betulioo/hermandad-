...
- Tienda: `/tienda`, `/tienda/[slug]`, `/tienda/carrito`, `/tienda/pedido`; carrito **Zustand** persistido; pedido sin pagos y con email de contacto para confirmación por Resend cuando esté configurado. El slice aprobado de stock valida disponibilidad antes de confirmar pedido, sin reservar/descontar stock al crear el pedido.
- **Eventos**: listado público `/eventos`, detalle `/eventos/[id]` con `notFound()` real, admin operativo en alcance MVP; fallback estático solo ante error de API; utilidades compartidas en `src/utils/event-format.ts`.
